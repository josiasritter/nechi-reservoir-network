...
function[obj_feval] = Cascade_HUB_vs17_train(optpars)

load inputdata_train;
...
%% function targetGen
    ...
    [targetGen_TOTAL, targetGen_TOT_adj, targetGen_G, targetGen_T, targetGen_P2, targetGen_P3] = ...
        targetGen_vs7_train(t,volumeTS_M,maxvol_M,volumeTS_G,maxvol_G,volumeTS_T,maxvol_T,volumeTS_pred_P2,maxvol_P2,volumeTS_pred_P3,maxvol_P3, ...
...
% 1. Objective function: Firm energy (MAX)
    ...
    firm_power_month = dCurve_P_monthly(p95_e(1));
...
% 2. Objective function: Average energy (MAX)
    ...
    [dCurve_P_yearly,pEmp] = durationCurve_vs2(avg_power_year_MW);
    total_energy_gwh = mean(powerTS_TOTAL)*24*(365*max(year_index)+3)/1000;
    avg_power_MW = mean(powerTS_TOTAL);
...
% 3. Objective function: Flood hazard (MIN)
    ...
    %Find yearly maxima
    max_HydrographTS = zeros(year_count,2);
    for n = 1 : year_count
        nnnn = year_index == n;
        z = max(HydrographTS(nnnn));
        max_HydrographTS(n,1) = z;
    end
    ...
    FHareas = zeros(year_count,1);
    for i = 1:year_count-1
        FHareas(i) = (max_HydrographTS(i,1)+max_HydrographTS(i+1,1))/2 * (max_HydrographTS(i,2)-max_HydrographTS(i+1,2));
    end
    FHareas(year_count) = max_HydrographTS(year_count,1)/2 * max_HydrographTS(year_count,2);
...
% 4. Objective function: Flow alteration (MIN)
    ...
    [FDC_all,pEmp] = durationCurve_vs2([HydrographTS,HydrographTS_nat]); % HydrographTS_nat required after checking with EPM
